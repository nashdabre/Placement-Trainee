...
@Composable
fun Quanti_quiz(navController: NavHostController) {
    val context = LocalContext.current
    val quizData= loadQuizFromJson(context, "logicalreasoning.json")
    ...
    if (selectedTopic == null) {
        TopicButtons(quizData.topics) { topic ->
            selectedTopic = topic
        ...
        ResultScreen(score = score, totalQuestions = selectedTopic?.questions?.size ?: 0) {
...
@Composable
fun TopicButtons(topics: List<Topic>, onTopicSelected: (Topic) -> Unit) {
    Column(
        Modifier
            .fillMaxSize()
            .fillMaxHeight()
...
@Composable
fun ResultScreen(score: Int, totalQuestions: Int, onRetakeQuiz: () -> Unit) {
    Column(
